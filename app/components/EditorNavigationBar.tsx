...
interface EditorNavigationBarProps {
    items: readonly EditorNavigationItem[];
    currentId: string;
...
const EditorNavigationBar = ({
    ...
    onToggleInspector,
    children
}: EditorNavigationBarProps) => {
    const { deviceSize } = useResponsive();

    const isMobile = deviceSize === 'mobile' || deviceSize === 'tablet';
...
const NotDesktopNav = ({ items, currentId, onChange }: Pick<EditorNavigationBarProps, 'items' | 'currentId' | 'onChange'>) => {
    return (
...
const DesktopNav = ({
    ...
    openInspector,
    onToggleInspector
}: Pick<EditorNavigationBarProps, 'items' | 'currentId' | 'onChange' | 'openInspector' | 'onToggleInspector'>) => {
    return (
        <View ui={cx(
            css`
                align-items: flex-start;
            ...
            {onToggleInspector && (
                <View ui={cx(
                    css`
                        ...
                        cursor: pointer;
                    `,
                    openInspector ? css` rotate: none; ` : undefined
...
interface NavItemProps extends ComponentPropsWithoutRef<'div'> {
    item: EditorNavigationItem;
    selected: boolean;
}

const NavItem = ({ item, selected, ...props }: NavItemProps) => {
    return (
        <View ui={cx(
            css`
                align-items: center;
            ...
            <Icon iconType={item.icon} width={24} height={24} ui={selected ? css`
                fill: var(--g-700);
            ` : css`
                fill: var(--g-400);
            `} />
