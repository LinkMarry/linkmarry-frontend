// import EditorInspectorWrapper from "~/feature/wedding-invitation/components/inspector/EditorInspectorWrapper";
// import {Column} from "~/components/core/FlexLayout";
// import Text from "~/components/components/Text";
// import Input from "~/components/components/Input";
// import PhotoUploadBox from "~/components/specific/PhotoUploadBox";
// import {css} from "@linaria/core";

const WeddingInvitationEditorAiInspector = () => {
    // return (
        // <EditorInspectorWrapper type={'ai'} toggle={{
        //     checked: true,
        //     OnChange: checked => {}
        // }}>
        //     <Column $alignItems={'stretch'} $gap={12}>
        //         <Text type={'p3'} bold={true}>디자인</Text>
        //         <Input/>
        //     </Column>
        //     <Column $alignItems={'stretch'} $gap={12}>
        //         <Text type={'p3'} bold={true}>사진 첨부</Text>
        //         <PhotoUploadBox id={'AiInspector-photo'} value={0} onChange={newValue => {}}/>
        //     </Column>
        //     <Column $alignItems={'stretch'} $gap={12}>
        //         <Text type={'p3'} bold={true}>미리보기</Text>
        //         <Column $alignItems={'stretch'} $gap={12} $ui={css`
        //             width: 300px;
        //         `}>
        //
        //         </Column>
        //     </Column>
        // </EditorInspectorWrapper>
    // );
    return null;
};

export default WeddingInvitationEditorAiInspector;
